...
photo_dir <- file.path(script_dir, "NEON_hemispheric-photos-veg", "stackedFiles", "tmp_photos")
...
for (i in seq_len(nrow(eventPlotData))){
  ...
  rm(img)
  print("Step 5/6")
  dest_jpg2 <- sub("\\.NEF$", "_hmsphr.jpg", dest_file)
  jpeg(dest_jpg2)
...
result_file <- file.path(script_dir, "canopy_results.csv")
write.csv(final_results, result_file)