...
#Usar los sitios seleccionados en perimagefilefinal
subdata_set_loc <- file.path(script_dir, "perimagefilefinal.csv")
subdata_set <- read.csv(subdata_set_loc)
#Seleccionar la segunda mitad.
n_sub <- nrow(subdata_set)
second_half_sub <- subdata_set[(floor(n_sub / 2) + 1):n_sub, ]

lessData <- subset(x = data, subset = data$uid %in% second_half_sub$uid)
...
#(j in seq_along(plot_list))
#(j in seq(102, length(plot_list)))
for(j in seq_along(plot_list)){
  ...
  #for (i in 11:nrow(eventPlotData))
